...
import { Plus, Trash2, Circle, CheckCircle2, Pencil, Check } from 'lucide-react'
...
// ─── Main Panel ───────────────────────────────────────────────────────────────
function TasksPanel({ isOpen, onClose, settings, accentColor }: TasksPanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-135 max-h-[80vh] rounded-xl overflow-hidden flex flex-col"
        style={{
      ...
      >
        ...
        {/* New Task Form */}
        {showForm && (
          <div className="px-6 py-4 border-b border-white/10 shrink-0" style={{ backgroundColor: 'rgba(255,255,255,0.03)' }}>
            <input
              autoFocus
              type="text"
              value={newTitle}
