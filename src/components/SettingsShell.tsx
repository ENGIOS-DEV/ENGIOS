...
export function SettingsShell({ title, isOpen, onClose, tabs, activeTab, onTabChange, accentColor, children }: SettingsShellProps) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div
        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[520px] max-h-[80vh] rounded-xl overflow-hidden flex flex-col"
        style={{
...
export function SliderRow({ label, value, min = 0, max = 100, onChange, accentColor }: {
  ...
  return (
    <div className="flex items-center gap-4 mb-3">
      <span className="text-sm text-white/70 w-32 flex-shrink-0">{label}</span>
      <input
